...
<template>
  <BaseModal title="从归档导入游戏库" :subtitle="sourcePath" width="600px" @close="emit('close')">
    <div v-if="loading" class="py-6 text-center text-[14px] text-ink-3">正在读取归档…</div>

    <div v-else-if="!summary" class="space-y-3">
      <p class="rounded-xl border border-line bg-surface-2 p-3 text-[13.5px] text-danger">
        {{ error || "无法读取该文件" }}
      </p>
    ...
    <div v-else class="space-y-4">
      <div class="rounded-xl bg-surface-2 p-3">
        <p class="text-[12.5px] text-ink-3">归档信息</p>
      ...
      <div>
        ...
        <div class="grid grid-cols-4 gap-2">
          ...
          >
            <p class="text-[15.5px] font-semibold text-ink">{{ summary[item.key] }}</p>
            <p class="mt-0.5 text-[12px] text-ink-3">{{ item.label }}</p>
          </div>
        </div>
      </div>
        ...
        <div class="space-y-2">
          ...
          >
            <span class="flex items-center gap-2">
              ...
              />
              <span class="text-[14px] font-medium text-ink">合并导入（推荐）</span>
            </span>
            <span class="mt-1.5 block pl-5 text-[13px] leading-relaxed text-ink-3">
          ...
          >
            <span class="flex items-center gap-2">
              ...
              />
              <span class="text-[14px] font-medium text-danger">覆盖导入</span>
            </span>
            <span class="mt-1.5 block pl-5 text-[13px] leading-relaxed text-ink-3">
      ...
      <p class="rounded-xl border border-line-soft bg-surface-2 p-3 text-[13px] leading-relaxed text-ink-3">
      ...
      <p v-if="error" class="rounded-xl border border-line bg-surface-2 p-3 text-[13.5px] text-danger">
        {{ error }}
      </p>
    </div>
